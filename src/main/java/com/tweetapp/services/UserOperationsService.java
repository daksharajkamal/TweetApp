...
public class UserOperationsService {
	...
	public UserModel findByUsername(final String username) {
		...
		return new UserModel(userModel.getUsername(), userModel.getFirstName(), userModel.getLastName(),
	...
	public List<UserModel> getAllUsers() {
		...
		return userModels.stream().map(user -> new UserModel(user.getUsername(), user.getFirstName(),
				user.getLastName(), user.getEmail(), "null", "null")).collect(Collectors.toList());
	}

	// Method to change a user's password
	public UserModel changePassword(final String username, final String newPassword, final String contact)
		...
		if (userDetails.getContactNum().equalsIgnoreCase(contact)
			...
			throw new PasswordMisMatchException(
					String.format("Two Passwords didnt match. unable to change password for user: %s", username));
		}
	}
