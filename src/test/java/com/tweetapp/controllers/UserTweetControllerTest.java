...
public class UserTweetControllerTest {
	...
	public void testGetAllTweets() throws Exception {
		final List<Tweet> tweets = Arrays.asList(
				new Tweet("1", "user1", "tweet1", "first", "last", new Date().toString(),
						Collections.singletonList("2"),
						Arrays.asList(new Comment("user2", "comment1"), new Comment("user2", "comment2"),
								new Comment("user2", "comment2"))),

				new Tweet("2", "user2", "tweet2", "second", "last", new Date().toString(),
		...
		RequestBuilder requestBuilder = MockMvcRequestBuilders.get("/tweets/all");
	...
	public void testPostTweet() throws Exception {
		Tweet postTweet = new Tweet("2", "user2", "tweet2", "second", "last", new Date().toString(),
				Collections.singletonList("2"), Arrays.asList(new Comment("user3", "comment1"),
						new Comment("user3", "comment2"), new Comment("user1", "comment1")));
		...
		RequestBuilder requestBuilder = MockMvcRequestBuilders.post("/tweets/{username}/add", "user2")
				.contentType(MediaType.APPLICATION_JSON).content(jsonObject).accept(MediaType.APPLICATION_JSON);
		MvcResult mvcResult = mockMvc.perform(requestBuilder).andReturn();
		MockHttpServletResponse response = mvcResult.getResponse();
	...
	public void testGetUserTweetsWithInvalidUserNameException() throws Exception {
		...
		Mockito.doThrow(new InvalidUsernameException("Username provided is invalid")).when(tweetService)
				.getUserTweets("user2", "loggedInUser");

		RequestBuilder requestBuilder = MockMvcRequestBuilders.get("/tweets/{username}", "user2")
				.header("loggedInUser", "loggedInUser").contentType(MediaType.APPLICATION_JSON)
